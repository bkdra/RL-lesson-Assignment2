...
DT = 0.025  # 每一步的時間間隔 (秒)
UPDATE_DISTANCE = 0.5  # 無人機距離當前目標點多近時切換到下一個目標點
...
class DroneROSInterface(Node):
    ...
    def teleport_to_pose(self, position):
        ...
        for attempt in range(10):
            ...
            self.get_logger().warning(
                f"/spawn_entity not ready yet, retrying ({attempt + 1}/10)"
            )

        if not service_ready:
            self.get_logger().error("/spawn_entity service not available after retries")
            return False
...
class DroneGymEnv(gym.Env):
    ...
    def __init__(self, ros_interface: DroneROSInterface):
        ...
        self.last_obs = np.zeros(3)  # 上一個 part of observation, 
                                      # only include 1. current pose
                                                   # 2. target position
                                                   # 3. the position drone should face to 
        self.curr_obs = np.zeros(13)  # 當前 part of observation
    ...
    def reset(self, seed=None, options=None):
        ...
        if not self.ros.teleport_to_pose(start_pos):
        ...
        self.last_obs = np.zeros(13)  # 上一個 part of observation, 
                                      # only include 1. current pose
                                                   # 2. target position
                                                   # 3. the position drone should face to 
        self.curr_obs = np.zeros(13)  # 當前 part of observation
        ...
        obs = self._get_obs(np.zeros(3))  # 初始 observation (動作全為 0)
    ...
    def step(self, action):
        ...
        if self.ros.current_pose[0] < self.x_bound[0] or self.ros.current_pose[0] > self.x_bound[1] or \
           ...
           self.ros.current_pose[2] < self.z_bound[0] or self.ros.current_pose[2] > self.z_bound[1]:
            ...
            terminated = True
            reward += len(self.randTraj) * 10  # 完成軌跡的額外獎勵
    ...
    def _compute_reward(self, obs, action):
        ...
        reward = 5000.0 * progress_reward - position_error - 0.1 * smoothness_penalty + position_reward + self.current_target_idx * 5.0
