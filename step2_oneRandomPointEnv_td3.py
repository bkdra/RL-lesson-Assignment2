...
class DroneGymEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.target_point = start_pos + np.array([rand_x, rand_y, rand_z])
    ...
    def _get_obs(self):
        ...
        next_target_vector = np.zeros(3)  # step2 is only one point
        next_next_target_vector = np.zeros(3)  # step2 is only one point
    ...
    def _compute_reward(self, action):
        ...
        reward = 100.0 * progress_reward - 0.5 * position_error - 0.7 * smoothness_penalty - 0.2 * np.linalg.norm(action)  # 總 reward
