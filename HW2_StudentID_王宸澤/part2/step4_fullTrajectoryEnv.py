...
class DroneGymEnv(gym.Env):
    ...
    def __init__(self, ros_interface: DroneROSInterface, traj_num=None):
        super().__init__()
        ...
        with open(base_dir / 'trajectory2_noFace_new.txt', 'r') as f:
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.traj_num is None:
            randnum = np.random.randint(0, 100) % 2
            if randnum == 1:
                self.traj2times += 1
                if self.traj2times > 2:
                    randnum = 0
                    self.traj2times = 0
            else:
                self.traj1times += 1
                if self.traj1times > 2:
                    randnum = 1
                    self.traj1times = 0
            if randnum == 0:
        ...
        self.next_target_point = np.zeros(3)  # step2 is only one point
        self.next_next_target_point = np.zeros(3)  # step2 is only one point

        self.x_bound = (np.min(self.randTraj[:, 0]) - 5, np.max(self.randTraj[:, 0]) + 5)
        self.y_bound = (np.min(self.randTraj[:, 1]) - 5, np.max(self.randTraj[:, 1]) + 5)
        self.z_bound = (np.min(self.randTraj[:, 2]) - 5, np.max(self.randTraj[:, 2]) + 5)

        self.ros.reset_world()
        self.ros.reset_drone()
        time.sleep(0.5)  # 等 Gazebo 穩定下來
    ...
    def step(self, action):
        ...
        if np.linalg.norm(self.ros.current_pose - self.randTraj[self.current_target_idx][:3]) < UPDATE_DISTANCE:
            ...
            self.notUpdateTimes = 0
            print(f"Target {self.current_target_idx} reached!")
            self.current_target_idx = self.current_target_idx + 1
            self.next_next_target_point = self.next_target_point.copy()
            self.next_target_point = self.target_point.copy()
            if self.current_target_idx < len(self.randTraj):
                self.target_point = self.randTraj[self.current_target_idx][:3]
        else:
            self.notUpdateTimes += 1
        ...
        elif self.notUpdateTimes > 2000:
            terminated = True
            reward -= 10.0  # 無更新的懲罰
            print("No update for too long!")
    ...
    def _compute_reward(self, action):
        ...
        reward = 100.0 * progress_reward - 0.5 * position_error - 0.1 * smoothness_penalty
        
        return reward

    def printstate(self, reward, action):
