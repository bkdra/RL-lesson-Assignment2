...
class FlyStraight(Node):
    ...
    def __init__(self):
        ...
        # ========== 2. 控制參數 ==========
        self.Kp = 0.2          # 比例增益 (Proportional gain)
        self.max_speed = 10.0   # 最大速度限制,單位: m/s
        ...
        self.target_x = self.trajectory2[0, 0]
        self.target_y = self.trajectory2[0, 1]
        self.target_z = self.trajectory2[0, 2]

        self.rise_to_height(target_z=self.target_z - 1.0)
    ...
    def rise_to_height(self, target_z=10.5, speed=3.0):
        ...
        while time.monotonic() - start < max_time:
            z = self.current_z if self.current_z is not None else 0.0

            if z >= target_z:
                break

            self.publish_velocity(0.0, 0.0, speed)
    ...
    def control_loop(self):
        ...
        if distance < self.tolerance:
            if not self.arrived:
                self.get_logger().info(
                    f'✅ 抵達{self.trajectory_index}! current position: '
                    f'({self.current_x:.2f}, {self.current_y:.2f}, {self.current_z:.2f})'
                )
                self.arrived = True
                self.trajectory_index += 1
                if self.trajectory_index < len(self.trajectory2):
                    self.target_x = self.trajectory2[self.trajectory_index, 0]
                    self.target_y = self.trajectory2[self.trajectory_index, 1]
                    self.target_z = self.trajectory2[self.trajectory_index, 2]
                else:
                    self.get_logger().info('🎉 已完成所有目標點!')
            # 抵達後發送停止指令 (速度全部設 0)
            self.publish_velocity(0.0, 0.0, 0.0)
            return
